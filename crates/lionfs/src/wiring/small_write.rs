...
impl<W: io::Write> SmallWriteRouter<W> {
    ...
    where
        F: FnMut(&LogEntry),
    {
        // One clone of the overlay's current entries is unavoidable
        // here: if `mark_checkpoint` below fails, `self.overlay` must
        // stay intact for the next drain attempt, so it cannot be
        // consumed until that call has succeeded. But the sink loop
        // no longer clones each payload a *second* time on top of
        // that -- `flat` is consumed by value, so each `Vec<u8>`
        // moves straight into the `LogEntry` the sink observes
        // instead of being cloned again.
        let mut flat: Vec<OverlayRec> = self.overlay.values().flatten().cloned().collect();
