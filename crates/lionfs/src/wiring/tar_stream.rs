...
///
/// Split into three flat slice sums instead of one 512-iteration loop
/// with a per-byte range check: the chksum field's contribution is a
/// compile-time constant (8 * b' '), and each `sum` below is a plain
/// reduction the compiler can autovectorize -- no per-byte branch on
/// the hot header-parse path (one call per archive member).
fn header_checksum(header: &[u8]) -> u64 {
    let sum = |bytes: &[u8]| -> u64 { bytes.iter().map(|&b| u64::from(b)).sum() };
    const CHKSUM_FIELD_AS_SPACES: u64 = 8 * (b' ' as u64);
    sum(&header[..148]) + CHKSUM_FIELD_AS_SPACES + sum(&header[156..])
}

/// Parses an octal ASCII field (leading spaces/NULs tolerated) into
/// a number. Returns `None` for non-octal content.
///
/// Single pass over the field, no intermediate allocation -- the
/// original collected the digit run into a `Vec<u8>` first, which
/// meant one small heap allocation per size/chksum/mtime field, i.e.
/// several per archive member on the import hot path.
...
///
/// Reads both fields as borrowed byte slices first and allocates
/// exactly one `String` for the result, instead of always building
/// two owned `String`s (name and prefix) and then a third via
/// `format!` when both are present -- on the common case (no ustar
/// prefix, which is the overwhelming majority of GNU-tar-produced
/// archives) this drops from two allocations to one.
fn header_name(header: &[u8]) -> Option<String> {
    let name = header[0..100].split(|b| *b == 0).next().unwrap_or(&[]);
    let prefix = header[345..500].split(|b| *b == 0).next().unwrap_or(&[]);
    if prefix.is_empty() {
