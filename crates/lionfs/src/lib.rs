...
// --- LionFS 8.0 (Unified: LFS × HFS) ----------------------------------------
//
// 8.0 merges the HelixFS (HFS) distributed plane into the LionFS engine as
// the `lionfs-cluster` crate (Raft consensus, CRDT namespace, CDC dedup,
// Reed-Solomon erasure coding, WAL/checkpoint recovery, convergent
// encryption, and the version-DAG time-travel engine) and adds in-engine
// bridges: `fs::timetravel` (resolve(path, t) over frozen snapshot trees),
// `guardian::reliability` (MTTDL / write-amplification math beside the
// Weibull predictor), `pipeline::cdc_dedup` (content-defined chunk dedup
// with domain-scoped convergent keys), and `pool::ec_volume` (standalone
// Reed-Solomon erasure-coded volumes built on the pool's own GF(256)
// codec).

/// Current release version of LionFS.
pub const VERSION: &str = "8.0.0";
/// Architecture edition name.
pub const EDITION: &str = "UNIFIED";
