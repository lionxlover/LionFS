...
///
/// The systematic encoding matrix is a pure function of `(n, k)` --
/// it is computed once here at construction and cached, instead of
/// being rebuilt (a k x k GF(256) matrix inversion, O(k^3), plus an
/// n x k x k multiply, O(n k^2)) on every [`Self::encode`] and
/// [`Self::reconstruct`] call. A pool doing erasure-coded writes or
/// repairs at a fixed profile was redoing that linear algebra from
/// scratch on every single write/repair before this cache; the
/// cluster plane's independent `ecc::RsCodec` already caches its
/// matrix the same way, which is what made this worth fixing here
/// too. `RsCode` therefore now owns heap data and is `Clone` but no
/// longer `Copy`/`PartialEq` -- nothing in the codebase compared two
/// `RsCode`s or relied on it being byte-copyable.
#[derive(Debug, Clone)]
pub struct RsCode {
    pub n: usize,
    pub k: usize,
    /// Cached systematic n x k encoding matrix (see [`build_encoding_matrix`]).
    matrix: Vec<Vec<u8>>,
}
...
impl RsCode {
    ...
    pub fn new(n: usize, k: usize) -> Self {
        ...
        let matrix = build_encoding_matrix(n, k);
        Self { n, k, matrix }
    }
    ...
    /// The systematic encoding matrix: an n x k matrix whose first k
    /// rows are the identity (data shards pass through) and whose
    /// remaining rows mix data into parity. Cached at construction
    /// (see [`build_encoding_matrix`]); this clones it out for
    /// callers that want their own copy.
    #[must_use]
...
/// Builds the systematic encoding matrix: an n x k matrix whose first
/// k rows are the identity (data shards pass through) and whose
/// remaining rows mix data into parity. Called once, from
/// [`RsCode::new`], and cached on the value from then on.
///
/// Construction (the standard Vandermonde-derived systematic form, as
/// used by Jerasure/Backblaze-class libraries): build the full n x k
/// Vandermonde `V`, invert its top k x k block `V_k`, and
...
fn build_encoding_matrix(n: usize, k: usize) -> Vec<Vec<u8>> {
    ...
    let top_inv = invert_gf256(&top, k);
    mat_mul(&v, &top_inv, n, k, k)
}
