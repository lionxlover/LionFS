...
impl SnapshotManager {
    ...
    /// Time travel (8.0): read one inode through a KNOWN record's frozen
    /// view. Path resolution walks many components; re-fetching the
    /// record per component (as `read_snapshot_inode` does) is wasted
    /// tree descent, so the timetravel module hands the record through.
    pub fn read_inode_in_record(
        ctx: &mut TxContext,
        record: &SnapshotRecord,
        ino: u64,
    ) -> Result<Option<Inode>> {
