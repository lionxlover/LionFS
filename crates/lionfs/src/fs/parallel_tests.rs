...
/// 8.0 regression money test (the silent-fsync-loss bug, found by the
/// time-travel suite): a fsync'd GROW-OVERWRITE of an existing file
/// (offset < old size, end > old size — the page-cache RMW path) must
/// survive both a clean unmount and a crash-style drop.
///
/// The bug: transaction ids are derived from `sb.generation + 1`, but
/// in-place-only commits never persist their tx id (no root cells → no
/// slot write), so a remount re-derived ids the journal still held.
/// `begin()` hands out the PRE-increment value, so the first new tx
/// REUSED the journal's highest id; recovery's duplicate-id map insert
/// then let the STALE journal entry replace the live one, and the
/// ordered replay applied the stale transaction LAST — the fsync'd
/// write reverted to the previous version. Fixed by the mount-time
/// generation checkpoint (WAL discipline), the off-by-one id floor,
/// the destroy-time tx-id floor, and first-scanned-wins duplicate
/// handling in the recovery scan.
#[test]
fn fsync_grow_overwrite_survives_clean_unmount() {
    let tag = "grow_overwrite_clean";
    {
        let mut fs = mount(tag, 64);
        let ino = fs
            .create(1, "doc.txt", &mkcreate())
            .expect("create")
            .ino;
    ...
    }
    {
        ...
        assert_eq!(attr.size, 11, "v1 committed size");
        fs.write(attr.ino, 0, b"version two (longer)")
            .expect("grow-overwrite");
        fs.fsync(attr.ino, true).expect("fsync v2");
        fs.destroy();
    }
    let fs = remount(tag);
    let attr = fs.lookup(1, "doc.txt").expect("lookup after remount");
    assert_eq!(
        attr.size, 20,
        "the fsync'd grow-overwrite must survive the clean unmount"
    );
    let data = fs.read(attr.ino, 0, 64).expect("read");
    assert_eq!(&data[..20], b"version two (longer)");
}

/// Same shape, crash-style drop (no destroy): the journal carries the
/// fsync'd group and recovery replays it — with NEW ids, so no stale
/// duplicate can overwrite it.
#[test]
fn fsync_grow_overwrite_survives_crash_drop() {
    let tag = "grow_overwrite_crash";
    {
        let fs = mount(tag, 64);
        let ino = fs
            .create(1, "doc.txt", &mkcreate())
    ...
    let fs = remount(tag);
    let attr = fs.lookup(1, "doc.txt").expect("lookup after crash");
    assert_eq!(
        attr.size, 20,
        "the fsync'd grow-overwrite must survive the crash drop"
    );
    let data = fs.read(attr.ino, 0, 64).expect("read");
    assert_eq!(&data[..20], b"version two (longer)");
}

/// Multi-round grow-overwrites across repeated remounts: ids keep
/// advancing (every remount checkpoints the recovery floor), the
/// journal never holds two entries under one id, and the file's
/// content is exactly the last fsync'd version.
#[test]
fn repeated_remount_grow_overwrites_never_revert() {
    let tag = "grow_overwrite_rounds";
    const VERSIONS: [&[u8]; 4] = [
        b"round zero",
        b"round one grows the file",
        b"round two grows the file even further",
        b"round three is the final and longest version of this log entry",
    ];
    {
        let mut fs = mount(tag, 64);
        let ino = fs
            .create(1, "log.txt", &mkcreate())
            .expect("create")
            .ino;
        fs.write(ino, 0, VERSIONS[0]).expect("write v0");
        fs.fsync(ino, true).expect("fsync v0");
        fs.destroy();
    }
    for (round, payload) in VERSIONS.iter().enumerate().skip(1) {
        let mut fs = remount(tag);
        let attr = fs.lookup(1, "log.txt").expect("lookup");
        fs.write(attr.ino, 0, payload).expect("overwrite");
        fs.fsync(attr.ino, true).expect("fsync");
        fs.destroy();
        let fs = remount(tag);
        let attr = fs.lookup(1, "log.txt").expect("re-lookup");
        assert_eq!(
            attr.size,
            payload.len() as u64,
            "round {round}: size must be the fsync'd version's"
        );
        let data = fs.read(attr.ino, 0, 4096).expect("read");
        assert_eq!(&data[..payload.len()], *payload, "round {round} content");
    }
}
