...
impl LionFS {
    pub fn new(mut disk: Disk, image_path: String) -> std::io::Result<Self> {
        ...
        let mut superblock = match crate::ondisk::superblock::pick_best(&candidates) {
            Some(sb) => sb,
            None => {
...
unknown feature bits {unknown:#b} set",
        ...
        // 8.0 durability fix (silent fsync loss on remount, found by the
        // time-travel money test): two compounding defects let a NEW
        // transaction reuse an id still resident in the journal, after
        // which recovery's duplicate-id resolution replayed the STALE
        // entry over the new one (a fsync'd grow-overwrite reverted to
        // the previous version on remount).
        //
        // (a) The id floor: `begin()` hands out the PRE-increment value,
        //     so seeding `current_tx_id = highest_tx` makes the first
        //     new tx REUSE the journal's highest id. Seed one ABOVE it.
        // (b) The WAL checkpoint discipline (the HFS engine's
        //     "replay, then immediately checkpoint" rule): persist
        //     `generation = highest_tx` to the superblock slots at
        //     mount, BEFORE any new transaction is numbered. A commit
        //     that moves no tree roots (pure in-place overwrite) never
        //     writes slots, so the on-disk generation can lag the
        //     journal's ids across sessions; without this checkpoint
        //     every remount replays old committed txs forever and the
        //     tool paths (bare TransactionManager over the on-disk sb)
        //     can number new txs into ids the journal still holds.
        if highest_tx > superblock.generation {
            let mut sb_floor = superblock;
            sb_floor.generation = highest_tx;
            crate::ondisk::superblock::write_all_slots(&disk, &sb_floor, highest_tx)?;
            superblock.generation = highest_tx;
        }
        ...
        // (a) continued: the next handed-out id must EXCEED every id the
        // journal ever held, or the first new transaction collides with
        // a stale journal entry under the same id.
        if highest_tx
            >= tx_manager
                .current_tx_id
                .load(std::sync::atomic::Ordering::SeqCst)
        {
            tx_manager
                .current_tx_id
                .store(highest_tx + 1, std::sync::atomic::Ordering::SeqCst);
        }
