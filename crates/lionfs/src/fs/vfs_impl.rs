...
impl VfsOps for LionFS {
    ...
    fn destroy(&mut self) {
        ...
        //
        // 8.0 durability fix: the checkpoint floor must ALSO cover the
        // highest transaction id this session numbered — commits that
        // move no tree roots (pure in-place overwrites) never persist
        // their tx id, and a stale on-disk generation is exactly what
        // let the next session reuse ids still resident in the journal
        // (the silent-fsync-loss bug). `current_tx_id` is the NEXT id
        // to hand out after the drain, so it is a safe upper bound.
        {
            let sb = self.core.sb();
            let tx_floor = self
                .core
                ...
                .saturating_sub(1);
            let floor = sb.generation.max(tx_floor);
            if let Err(err) =
                crate::ondisk::superblock::write_all_slots(&self.core.disk, &sb, floor)
            {
