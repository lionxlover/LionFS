                ...
                if compute_checksum(bytes_of(&header)) == saved_checksum {
                    ...
                    if valid && temp_block < sb.journal_blocks {
                            ...
                            && compute_checksum(bytes_of(&footer)) == footer_checksum
                        {
                            ...
                            //
                            // 8.0 hardening: duplicate tx ids in the
                            // journal mean a session reused an id (the
                            // mount-time generation checkpoint now
                            // prevents this) or a torn/corrupt region.
                            // Keep the FIRST-scanned entry — an
                            // `insert` here would silently let a stale
                            // duplicate (scanned later, deeper in the
                            // wrapped log) replace the live one, which
                            // is exactly the replay-stale-over-new
                            // data-loss shape the 8.0 durability fix
                            // closed.
                            tx_to_replay
                                .entry(header.tx_id)
